    ...
    <div className="min-h-screen flex flex-col items-center justify-start pt-28 pb-16 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7 w-full max-w-5xl px-2">
        {pagedProducts.length === 0 ? (
          <div className="col-span-full text-center text-sky-200 text-lg">Không tìm thấy sản phẩm phù hợp.</div>
        ) : (
        ...
        <div className="flex gap-2 mt-10">
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              key={i}
              onClick={() => setPage(i + 1)}
              className={`px-4 py-2 rounded-lg font-bold shadow ${page === i + 1 ? "bg-[#233658] text-white" : "bg-white text-[#233658] hover:bg-sky-100"}`}
            >
              {i + 1}
            </button>
          ))}
